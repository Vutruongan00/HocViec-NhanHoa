# **Tìm hiểu Các loại Logs quan trọng trên Linux**
...
## 2 **Log dịch vụ cụ thể (Service-specific Logs)**
...
### 2.5. **Log đăng nhập (Authentication/Login Logs)**
- **Đường dẫn chính**
...
-  **Chức năng** : Ghi nhận tất cả hoạt động liên quan đến xác thực: đăng nhập, sudo, su, SSH login, v.v.
  
-  **Lệnh ví dụ dùng để phát hiện đăng nhập sai:**

   ``` grep "Failed password" /var/log/auth.log ```

### 2.6.  **Log reboot/shutdown (Khởi động/Tắt máy)**
- **Công cụ sử dụng: last**
...
#### 3.1.1. **Cách Lọc log theo tiêu chí cụ thể**
   a. **Theo dịch vụ -u (unit)**
...
![](Aspose.Words.2de371a4-6a42-46cd-8a82-542887225f0d.004.png)
   ...
   b. **Theo mức đọ ưu tiên (-p) (priority)** 
...
- Thư mục cấu hình chi tiết theo dịch vụ: 
  ...
  ### 3.2.1. **Ví dụ cấu hình đơn giản trong mysql:**
...
- rotate 7 : Giữ lại **7 bản log cũ**, sau đó xóa các bản cũ hơn
- missingok: **Không báo lỗi** nếu file log không tồn tại
- create 640 mysql adm: Sau khi xoay log, tạo **file log mới** với:quyền 640 (chỉ owner đọc/ghi, group đọc, người khác không truy cập)- owner: msql – group: adm

### 3.2.2. **Các tùy chọn thường dùng**
...
- Rsyslog là một trong những daemon syslog phổ biến nhất, được biết đến với hiệu suất cao và tính linh hoạt. Nó hỗ trợ gửi log qua TCP và UDP, có thể cấu hình để lưu log vào nhiều định dạng khác nhau, và hỗ trợ các tính năng như lọc log, gửi log từ xa, và nhiều hơn nữa.
  #### 3.4.1. Cấu hình chính của rsyslog
