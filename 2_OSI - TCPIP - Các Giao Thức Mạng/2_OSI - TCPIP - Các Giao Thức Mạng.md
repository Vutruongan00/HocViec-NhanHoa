`		`**BÁO CÁO KẾT QUẢ CÔNG VIỆC NGÀY 22/4**

1. **NHỮNG VIỆC ĐƯỢC TRIỂN KHAI**
- Tìm hiểu 2 mô hình OSI và TCP/IP và so sánh chúng
...
1. **Nguyên tắc định nghĩa các tầng hệ thống mở**
...
1. **Vai trò và chức năng chính các tầng**
...
1. **Mô hình phân tầng trong TCP/IP**
...
1. **Cách thức hoạt động của TCP/IP**
...
1. **Giống nhau:** Mô hình OSI và TCP/IP có một số điểm chung như sau:
...
2. **Phân loại giao thức mạng (Theo mô hình OSI - 7 tầng)**
...
1. **Tầng Liên kết Dữ liệu (Data Link Layer)**
...
2. **Một số giao thức phổ biến thường dùng trong thực tế**
   1. **Trong WEB**
